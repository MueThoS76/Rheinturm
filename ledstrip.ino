...
#define CLOCK_LED_SEKUNDEN CRGB::Yellow;
#define CLOCK_LED_MINUTEN  CRGB::Yellow;
#define CLOCK_LED_STUNDEN  CRGB::Yellow;
...
void display_clock() {
  for (int i = LED_SEKUNDEN_EINER_MIN; i <= LED_SEKUNDEN_EINER_MAX; i++) {
    if (i - LED_SEKUNDEN_EINER_MIN < sekunden.einer) {
      ClockLeds[i] = CLOCK_LED_SEKUNDEN;
    } else {
